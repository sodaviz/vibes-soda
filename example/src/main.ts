...
export function f(params: Params) {

    let bacteriaNames = params.bacteriaNames;
    let phageNames = params.phageNames;
    let phageLengths = params.phageLengths;
    let bacteriaLengths = params.bacteriaLengths;
    let integrationAnnotations = params.integrationAnnotations;
    let virusGeneAnnotations = params.virusGeneAnnotations;
    let bacteriaGeneAnnotations = params.bacteriaGeneAnnotations;
    ...
    let occurrencesChart = new soda.Chart<VirusRenderParams>({
        ...
        draw(params): void {
            ...
            let numMosaicRows = 0;
            for (const ann of params.mosaic) {
                numMosaicRows = Math.max(numMosaicRows, ann.row);
            }
            numMosaicRows += 1;
